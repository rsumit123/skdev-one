...
(async()=>{
  ...
  await p1.click('#clSolo');
  const spun=await p1.waitForFunction(()=>document.getElementById('clSolo').classList.contains('busy'),{timeout:3000}).then(()=>true).catch(()=>false);
  ...
  ok(await p1.evaluate(()=>!document.getElementById('clSolo').classList.contains('busy')),'spinner clears after success');
  ...
  ok(await p2.evaluate(()=>!document.getElementById('clSolo').classList.contains('busy')),'button re-enabled so you can retry');
